...
app_function (void *userdata)
{
  ...
  /* Build pipeline */
  data->pipeline =
      gst_parse_launch ("udpsrc port=5000 ! application/x-rtp, encoding-name=H264,payload=96 ! rtph264depay ! avdec_h264 ! videoconvert ! autovideosink sync=false", &error);
      // gst_parse_launch ("ahcsrc ! videoconvert ! autovideosink", &error);
  if (error) {
    gchar *message =
...
JNI_OnLoad (JavaVM * vm, void *reserved)
{
  ...
  jclass klass = (*env)->FindClass (env,
      "com/archrinto/robocovid/RemoteActivity");
